...
for idx, selected_patient in enumerate(top_6_patients):
    ...
    for frame_idx, file in enumerate(patient_files):
        ...
        # Write frame with bounding box to video
    ...
    # Check if pixel_array is empty
    if not pixel_array:
        print(f"No valid images left for patient {selected_patient} after processing. Skipping PCA and video generation.", flush=True)
        continue

    # Perform PCA for the patient
    images_closed = []
    for image in pixel_array:
        resized_image = transform.resize(image, (image.shape[0] // down_sample_rate, image.shape[1] // down_sample_rate))
        closed_image = morphology.closing(resized_image, footprint)
        images_closed.append(closed_image)

    # Check if images_closed is empty
    if not images_closed:
        print(f"No valid processed images for PCA for patient {selected_patient}. Skipping PCA.", flush=True)
        continue

    # Convert images_closed to a NumPy array for PCA calculations
    images_closed = np.array(images_closed)
    images_c = ((images_closed - np.mean(images_closed)) / np.std(images_closed)).reshape(len(images_closed), -1).T
    XXT = np.matmul(images_c.T, images_c)
    eigen_values_nn, eigen_vectors_nn = np.linalg.eig(XXT)
    eigen_vectors_dn = np.matmul(np.matmul(images_c, eigen_vectors_nn), np.linalg.inv(np.diag(eigen_values_nn)))
    ys = np.matmul(images_c.T, eigen_vectors_dn[:, 0])  # First principal component
    ...
    for frame_idx, file in enumerate(patient_files):
        ...
        plt.plot(xs, ys, label=f"Patient {selected_patient}")
        plt.scatter(frame_idx, ys[frame_idx], color='red', label="Current Frame")
        plt.title(f"PCA Output for Patient {selected_patient}")
        plt.xlabel("Frame Index")
        plt.ylabel("Normalized PCA Output")
        plt.legend()
        plt.tight_layout()
    ...
    plt.ylabel("Normalized PCA Output")
    plt.legend()
    plt_path = os.path.join(output_dir, f"Patient_{selected_patient}_PCA.png")
    plt.savefig(plt_path)
    plt.close()

    print(f"Saved PCA plot and video for patient {selected_patient} in {output_dir}", flush=True)
