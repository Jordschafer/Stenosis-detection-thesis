...
for idx, selected_patient in enumerate(top_6_patients):
    ...
    # List to store bounding box metrics
    bounding_box_areas = []
    ...
    for frame_idx, file in enumerate(patient_files):
        ...
        try:
            ...
            # Calculate bounding box area and store it
    ...
    # Normalize the bounding box areas for plotting
    if bounding_box_areas:
        bounding_box_areas = np.array(bounding_box_areas)
        normalized_areas = (bounding_box_areas - np.min(bounding_box_areas)) / (np.max(bounding_box_areas) - np.min(bounding_box_areas))
        xs = np.arange(0, len(normalized_areas))
    ...
    for frame_idx, file in enumerate(patient_files):
        ...
        plt.plot(xs, normalized_areas, label=f"Patient {selected_patient}")
        plt.scatter(frame_idx, normalized_areas[frame_idx], color='red', label="Current Frame")
        plt.title(f"Bounding Box Area Changes for Patient {selected_patient}")
    ...
    # Plot the bounding box area changes
    plt.figure(figsize=(8, 5))
    plt.plot(xs, normalized_areas, label=f"Patient {selected_patient}")
    plt.title(f"Bounding Box Area Changes for Patient {selected_patient}")
    plt.xlabel("Frame Index")
    plt.ylabel("Normalized Bounding Box Area")
    plt.legend()
    plt_path = os.path.join(output_dir, f"Patient_{selected_patient}_Bounding_Box_Area.png")
    plt.savefig(plt_path)
    plt.close()

    print(f"Saved bounding box area plot and video for patient {selected_patient} in {output_dir}", flush=True)
